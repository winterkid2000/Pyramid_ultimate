...
class PyramidApp:
    ...
    def start_pipeline(self):
        ...
        if organ == 'pancreas':
            ...
            if not run_combined_descriptor_liv(nifti_output, nii_path, stl_path, output_xlsx):
                return       
        elif organ == 'lung':
            if not run_combined_descriptor_lung(nifti_output, nii_path, stl_path, output_xlsx):
                return 
        elif organ == 'kidney':
            if not run_combined_descriptor_kidn(nifti_output, nii_path, stl_path, output_xlsx):
                return 
        elif organ == 'spleen':
            if not run_combined_descriptor_spl(nifti_output, nii_path, stl_path, output_xlsx):
                return
        else:
            self.log('존재하지 않는 장기입니다...')
        ...
        try:
            if organ == 'pancreas':
                ...
                predict_with_model_liv(output_xlsx, model_path, scaler_path, log_callback=self.log)
            elif organ == 'lung':
                predict_with_model_lung(output_xlsx, model_path, scaler_path, log_callback=self.log)
            elif organ == 'kidney':
                predict_with_model_kidn(output_xlsx, model_path, scaler_path, log_callback=self.log)
            elif organ == 'spleen':
                predict_with_model_spl(output_xlsx, model_path, scaler_path, log_callback=self.log)
            else:
                self.log('존재하지 않는 장기입니다...')
